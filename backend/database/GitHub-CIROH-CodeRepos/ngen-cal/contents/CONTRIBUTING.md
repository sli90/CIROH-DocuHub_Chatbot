...
Note that this document only discusses aspects of Git usage that should be needed for day-to-day development and contributions.  For a more detailed overview of the project's use of Git, see the [GIT_USAGE](doc/GIT_USAGE.md) doc.

## Summary

To work with the repo and contribute changes, the basic process is as follows:

- Set up your [fork and local development clone](#getting-started)
- [Keep your fork and local clone up to date](#keeping-forks-up-to-date) with official OWP repo
  - Use [rebasing](#rebasing-development-branches) to keep histories consistent
- Create feature/fix branches from `master` when you want to contribute
- Write changes you want to contribute
  - Commit to your local feature/fix branch
  - Push these commits to a branch in your fork
- Submit pull requests to the official OWP repo's `master` branch
  - Watch for any requested changes during review processes
...
After that, [clone a local development repo](https://docs.github.com/en/repositories/creating-and-managing-repositories/cloning-a-repository) from your fork, using a command similar to one of the following:
    ...
    git clone git@github.com:your_user/ngen-cal.git

    # HTTPS-based clone command.  Change URL to match your fork as appropriate
    git clone https://github.com/your_user/ngen-cal.git
...
You can now change directories into the local repo, which will have the _default_ branch - `master` for this repository - checked out.
    ...
    # Move into the repo directory "ngen-cal"
    cd ngen-cal
...
Next, add the OWP repo as a second [Git remote](https://git-scm.com/book/en/v2/Git-Basics-Working-with-Remotes) for the local clone.  Doing the addition will look something like:
    ...
    git remote add upstream https://github.com/NOAA-OWP/ngen-cal.git
...
* [Work in a Dedicated Branch](#work-in-a-dedicated-branch)
  * [Creating a Feature/Fix Branch](#creating-a-feature-or-fix-branch)
  * [Setup Python Environment](#setup-python-environment)
  * [Start Committing Changes](#start-committing-changes)
...
Because Python is used, you will need to setup and activate a Python virtual environment:
...
You will also need to activate the environment again (if it isn't already active) whenever running the repo's Python packages or their tests.

To make sure the Python environment always reflects the current source code, install development version of the repo's package(s):

    # Note this all assume your still in an activated environment
...
Once the feature/fix branch is in place, begin writing and committing your changes to it.  While up to you, it is suggested that development work be committed frequently when changes are complete and meaningful. If work requires modifying more than one file in the source, it is recommended to commit the changes independently to help avoid too large of conflicts if/when they occur.

We recommend commit messages follow [conventional commits](https://www.conventionalcommits.org/en/v1.0.0/#summary) style. This is not a requirement, but adherence is appreciated :).
...
* Ensure any changes or additions to functionality are tested
  * If the pull request introduces code or changes to existing code, automated tests should be included
  * These tests should be written so they are runnable by `pytest`
